...
**Audio** is prerecorded into static MP3s in `static/audio/` (so playback needs no
in-browser TTS) by `tools/dump-audio-texts.mjs` (collects seed/passage text) →
`tools/gen-audio.py` (synthesizes via **edge-tts**: zh-CN / ja-JP / he-IL neural
voices). `static/audio/manifest.json` maps language → text → clip id; the play
button looks up and plays the clip, falling back to on-device TTS only if missing.
